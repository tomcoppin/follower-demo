...
*	\return none
*/
Raven_Controller::Raven_Controller() : it_(nh_)
{
...
bool Raven_Controller::init_ros(int argc, char** argv) 
{
	...
	//static ros::NodeHandle n;
  	RavenAutomove_publisher = nh_.advertise<raven_automove>("raven_automove", 1);
	RavenState_subscriber   = nh_.subscribe("ravenstate",1,&Raven_Controller::callback_raven_state,this);


    image_sub_ = it_.subscribe("/image_raw", 1, &Raven_Controller::imageCb, this);
	...
	if(ros::isInitialized())
	{
		...
		while(ros::ok() && RECEIVED_FIRST)
		{
			...
			else 
			{
				...
				//TF_INCR[LEFT_ARM] = LEFT_PATH.ComputeNullTrajectory(); 
				TF_INCR[LEFT_ARM] = LEFT_PATH.ComputeNullTrajectory();
				TF_INCR[RIGHT_ARM] = RIGHT_PATH.ComputeFollowTrajectory();
...
void Raven_Controller::extract_loc(int lb, int ub, int feature, const sensor_msgs::ImageConstPtr& msg)
{
...
inRange(hue, Scalar(lb), Scalar(ub), threshold_output);
/// Find contours
...
void Raven_Controller::imageCb(const sensor_msgs::ImageConstPtr& msg)
{
